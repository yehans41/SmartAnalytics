...
- **Date Range**: {summary.get('date_range', {}).get('start', 'N/A')} to \
{summary.get('date_range', {}).get('end', 'N/A')}
...
"""
        ...
        for col, info in outlier_info.items():
            if info["count"] > 0:
                ...
                md += f"| {col} | {info['count']:,} | "
                md += f"{info['percentage']:.2f}% | {bounds} |\n"
            ...
            if validation_results["failed"] > 0:
                ...
                for result in validation_results["results"]:
                    if not result["passed"]:
                        md += f"| {result['metric']} | {result['value']:.4f} | "
                        md += f"{result['threshold']:.4f} | "
                        md += f"{result['message']} |\n"
        ...
        if null_info["total_nulls"] > 0:
            null_count = null_info["total_nulls"]
            md += f"- **Missing Values**: {null_count:,} null values detected. "
            md += "Consider imputation or removal.\n"

        total_outliers = sum(info["count"] for info in outlier_info.values())
        if total_outliers > 0:
            md += f"- **Outliers**: {total_outliers:,} outliers detected "
            md += "across numeric columns. Review and cap if necessary.\n"
